 ...
 *  - Gradient/clip ids derive from useId, not Math.random, so server
 *    and client markup match (Next SSR hydration).
 *  - Tooltip/label surfaces use Velur tokens directly; --chart-* CSS
 *    vars live in app/globals.css.
 */
...
function DateTicker({ currentIndex, labels, visible }: DateTickerProps) {
  ...
  return (
    <motion.div
      className="overflow-hidden rounded-full bg-velur-ink px-4 py-1 text-white shadow-lg"
      layout
      transition={{
        ...
        <div className="space-y-1.5">
          {rows.map((row) => (
            ...
            >
              ...
              <span className="font-sans font-medium text-sm text-white tabular-nums">
  ...
  return createPortal(
    ...
    >
      <motion.div
        ...
        className="min-w-[140px] overflow-hidden rounded-lg bg-velur-ink/90 text-white shadow-lg backdrop-blur-md"
...
function XAxisLabel({
  ...
  return (
    ...
    >
      <motion.span
        animate={{ opacity }}
        className={cn("whitespace-nowrap font-mono text-[11px] text-slate")}
        initial={{ opacity: 1 }}
  ...
  return createPortal(
    <div className="pointer-events-none absolute inset-0">
      {ticks.map((tick) => (
        ...
        >
          <span className="whitespace-nowrap font-mono text-[11px] text-slate tabular-nums">
            {tick.label}
          </span>
        </div>
      ))}
...
export function Area({
  ...
  return (
    <>
      ...
      <g
        clipPath={
          animate ? `url(#grow-clip-area-${dataKey}-${uniqueId})` : undefined
        }
      >
        <motion.g
